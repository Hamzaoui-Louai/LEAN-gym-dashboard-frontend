...
- Contexts follow an unusual split: the provider lives in `src/context/` (`AuthContext.jsx`, `BackgroundContext.jsx`), while the `createContext` object + `useX` hook live in `src/hooks/` (`useAuth.js`, `useBackground.js`). Exception: `PageTransitionProvider` lives in `src/components/PageTransition.jsx` with its hook in `src/hooks/usePageTransition.js`. Keep new contexts on this pattern.
- Dashboard module pages are placeholder shells: each renders `PageHeader` + `PlaceholderBody` from `src/components/` with a "coming soon" note — the actual module features aren't built yet. Build them out by replacing those shells.
