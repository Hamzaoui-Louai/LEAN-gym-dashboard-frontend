...
import { GymProfileSkeleton } from '../components/Skeletons'
...
function GymProfilePage() {
  ...
  const isLoading = isLive && (gymPending || membersPending || staffPending || equipmentPending)
  const gym = rawGym ?? EMPTY_GYM

  useEffect(() => {
    if (isLive && gymError) {
      setToastMsg("Couldn't load gym data. Check your connection and try again.")
    }
  }, [isLive, gymError])
  ...
  const handleSave = async (next) => {
    try {
      ...
      setToastMsg('Failed to save gym profile. Please try again.')
    } finally {
      setIsEditOpen(false)
    }
  }
            ...
            <div className="flex flex-col gap-5 p-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="flex min-w-0 items-start gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-3 text-sm text-white/50">{gym.address || PLACEHOLDER}</p>
                  <p className="mt-1 text-xs text-white/40">
                    Registered {gym.registered_at ? formatDate(gym.registered_at) : PLACEHOLDER}
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-6 py-4">
              ...
              <div className="mt-2 divide-y divide-white/5">
                <InfoRow
                  label="Opening time"
                  value={gym.opens_at || PLACEHOLDER}
                  icon={
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
                ...
                <InfoRow
                  label="Closing time"
                  value={gym.closes_at || PLACEHOLDER}
                  icon={
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
                ...
                <InfoRow
                  label="Days open"
                  value={gym.days_open?.length ? formatDays(gym.days_open) : PLACEHOLDER}
                  icon={
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
